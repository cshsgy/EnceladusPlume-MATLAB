...
% re-cast: h static, w static
h_rec = zeros(length(h_rec),1);
w_rec = ones(length(w_rec),1)*0.01;
...
while(t<Periods_Run*Period)
    ...
    if ~isreal(Tw)
        error('ERROR: Tw becomes complex number or NaN. Error must have been occurred.');
    end
    T_rec{end+1} = T;
