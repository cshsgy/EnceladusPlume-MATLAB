...
The gas lookup table, the w_eff* grid and the adopted fit results are committed
under [`../results/`](../results/README.md) (`run_fit.DEFAULT_LOOKUP`,
`run_fit.DEFAULT_RESULT`, ...), so the figures can be redrawn without refitting;
the scripts below still write their larger intermediate products (chains, Monte-Carlo
realizations) under the system temp dir (`/tmp/...`). The observed
...
| `two_modes.py` | Local optimization around each of the two fit optima (mode A: φ₂≈84°; mode B: φ₂≈123°), with fit-overlay figures for visual comparison. Equivalent to `run_fit.py --refine` for mode B. | `../results/diurnal_fit_modeA.json`, `../results/diurnal_fit.json`, `/tmp/mode{A,B}_fit.pdf` |
...
| `gen_figs.py` | Regenerates the ensemble figure (Fig 10) and the corner figure (Fig 11) for the adopted fit (`../results/diurnal_fit.json`), with the truth-cross at mode B. | `diurnal_fit_ensemble.pdf`, `diurnal_fit_posterior.pdf` in the paper repo |
...
The adopted best fit is mode B (Δδ≈7 mm, L≈8 km, φ₂≈123°, α≈0.81, σ_φ≈32°,
